    	...
    	<div class="card mt-3 shadow ">
    		<div class="card-body">
    			<c:if test="${professeur != null}">
    				<form action="modifier_prof" method="post">
    			</c:if>
    			<c:if test="${professeur == null}">
    				<form action="inserer_prof" method="post">
    			</c:if>

    			<c:if test="${professeur != null}">
    				<input type="hidden" name="prof_id" value="<c:out value='${professeur.prof_id}' />" />
    			</c:if>

    			<div class="form-group">
    				<label for="">Anarana</label>
    				<input type="text" value="<c:out value='${professeur.prof_nom}'/>" class="form-control" name="prof_nom" required />
    				<label for="">Fanampiny</label>
    				<input type="text" value="<c:out value='${professeur.prof_prenom}'/>" class="form-control" name="prof_prenom" required  />
    			</div>
